...
if  [ ! -n "$UMI_ENV" ] ;then
    output="umi-test-zip-dist"
else
    output="${UMI_ENV}-umi-test-zip-dist"
fi

# 删除之前打包好的项目

rm -rf ./*-zip-dist ./dist ./umi-test*.zip
mkdir ./${output}
mkdir ./${output}/bin
...
if test -n "$UMI_ENV"; then
  if [ "$(uname)" == "Darwin" ];then
    sed -i '' 's/umi-test/'${UMI_ENV}'_umi-test/g' ./${output}/package.json
  else 
    sed -i 's/umi-test/'${UMI_ENV}'_umi-test/g' ./${output}/package.json
  fi
fi
...
if test -n "$UMI_ENV"; then
  if [ "$(uname)" == "Darwin" ];then
    sed -i '' 's/umi-test/'${UMI_ENV}'_umi-test/g' ./${output}/bin/stop.sh
  else 
    sed -i 's/umi-test/'${UMI_ENV}'_umi-test/g' ./${output}/bin/stop.sh
  fi
fi
